...
impl <T> SendPtr<T> {
    fn as_slice(&self, size: usize) -> &[T] {
        unsafe {
            std::slice::from_raw_parts(self.ptr, size)
        }
    }

    fn as_slice_mut(&self, size: usize) -> &mut [T] {
        unsafe {
            std::slice::from_raw_parts_mut(self.ptr, size)
        }
    }
}
...
impl<T> Clone for SendPtr<T> {
    fn clone(&self) -> Self {
        return SendPtr{ptr: self.ptr};
    }
}

impl<T> Copy for SendPtr<T> {}

impl<T, const CHUNK_COUNT: usize> AVec<'static, T, CHUNK_COUNT> {
    const CHUNKS: usize = CHUNK_COUNT;
    const MASK : usize = CHUNK_COUNT - 1;
    pub fn new(size: usize) -> AVec<'static, T, CHUNK_COUNT> {
        ...
        let send_ptr = SendPtr{ptr: chunks_raw};

        let (tx, grow_thread) = Self::start_grow_thread(send_ptr.clone());
    ...
    fn start_grow_thread(chunks_send: SendPtr<FVec<T>>) -> (Sender<(usize, usize)>, JoinHandle<()>){
        let (tx, rx) = mpsc::channel::<(usize, usize)>();
        let handle = thread::spawn(move || {
            let mut replace_index = 0;
            let mut largest_index = CHUNK_COUNT - 1;
            let chunks = chunks_send.as_slice_mut(CHUNK_COUNT); // They will never overlap so it is fine
